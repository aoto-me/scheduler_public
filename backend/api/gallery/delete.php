...
function deleteCard(int $userId, int $id, string $target): void
{
    handleDeleteRequest(function (PDO $pdo) use ($userId, $id, $target) {
        ...
        $galleryId = $inputData['postId'] ?? null;
        ...
        // 全アイテムの削除
        $itemResult = deleteRecord($pdo, $itemTable, ['galleryId' => $galleryId, 'cardId' => $id, 'user' => $userId]);
        if ($itemResult === false) {
            throw new HttpException("{$itemTable}：削除に失敗しました", 500);
        }

        // カードの削除
        $cardResult = deleteSingleRecord($pdo, $cardTable, ['id' => $id, 'galleryId' => $galleryId, 'user' => $userId]);
        if ($cardResult === false) {
